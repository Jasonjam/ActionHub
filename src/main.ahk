...
if (!A_IsCompiled) { ; 如果非exe = 是main.ahk，把路徑拉到src的上一層
    loop files, A_ScriptDir "\..", "D"
        APP_ROOT := A_LoopFileFullPath
}
global SRC_DIR := APP_ROOT "\src"
global ASSET_DIR := APP_ROOT "\assets"

; --- Tray Icon ---
global trayIcon := ASSET_DIR "\app_icon.ico"
global trayIcon_suspend := ASSET_DIR "\app_icon_suspend.ico"
; 檢查 檔案是否存在
if !FileExist(trayIcon) {
    MsgBox("找不到 TrayIconOn:n" trayIcon, "Icon Error", 16)
    ExitApp()
}
if !FileExist(trayIcon_suspend) {
    MsgBox("找不到 TrayIconOff:n" trayIcon_suspend, "Icon Error", 16)
    ExitApp()
}
TraySetIcon(trayIcon) ; 設定托盤圖示
...
subMenu_edit := Menu() ; 定義 子選單
subMenu_edit.Add("編輯動作檔", EditFile.Bind("action"))
subMenu_edit.Add("編輯設定檔", EditFile.Bind("setting"))
Tray.Add("編輯", subMenu_edit)  ; 渲染 子選單
Tray.Add() ; ---分隔線---

subMenu_reset := Menu() ; 定義 子選單
subMenu_reset.Add("若改壞，再使用以下功能", (*) => 0)
subMenu_reset.Disable("1&") ; 灰掉 + 不能點。引號內的 1& 代表第1項的索引
subMenu_reset.Add() ; ---分隔線---
subMenu_reset.Add("重置動作檔(Action Func)", ResetFile.Bind("action"))
subMenu_reset.Add("重置設定檔(視窗 && 熱鍵)", ResetFile.Bind("setting"))
Tray.Add("重置 (警告)", subMenu_reset)  ; 渲染 子選單
Tray.Add() ; ---分隔線---

suspendLabel := "鎖定熱鍵 (C+F8)"
Tray.Add(suspendLabel, ToggleSuspend)
...
try {
    sectionText := IniRead(iniFilePath, "Hotkey")
} catch {
    try {
        sectionText := IniRead(iniFilePath, "default-hotkey")
    } catch {
...
; --- 編輯檔案 ---
EditFile(fileType, *) {
    global actionFilePath, iniFilePath
    ...
    try {
        ...
        MsgBox("無法開啟檔案：`n" FilePath "`n`n原因：" err.Message, "錯誤", 16)
    }
}
...
ResetFile(fileType, *) {
    ...
    if (fileType == "action") {
        ResetFileCore(defaultActionFile, actionFilePath, "動作檔")
        return
    }
    if (fileType == "setting") {
        ResetFileCore(defaultIniFile, iniFilePath, "設定檔")
        return
    }
    ; 核心邏輯: 確認詢問 & 覆蓋
    ResetFileCore(defaultFile, targetFile, fileLabel) {
        result := MsgBox(
            "確定要重置" fileLabel "嗎?`n`n"
            . "目標檔案:`n" targetFile "`n`n"
            . "這會覆蓋目前內容",
            "確認重置",
            "OKCancel Icon!"
        )
        if (result != "OK") ; 取消
            return
        if !FileExist(defaultFile) { ; default檔案不存在，錯誤
            MsgBox("找不到預設檔案:`n" defaultFile, "錯誤", 16)
            return
        }

        ; 覆蓋檔案
        try {
            FileCopy(defaultFile, targetFile, 1)
            MsgBox(fileLabel "已重置，將重新載入腳本", "完成", 64)
            Reload()
        } catch as err {
            MsgBox("重置" fileLabel "失敗：`n" err.Message, "錯誤", 16)
        }
    }
}
...
ProcessSave(rows) {
    ...
    ; 這裡寫入處理好的大字串
    IniWrite(finalData, iniFilePath, "Hotkey")
...
; --- 切換 鎖定熱鍵 狀態 ---
ToggleSuspend(*) {
    global suspendLabel, trayIcon_suspend, trayIcon
    Suspend(-1) ; 切換掛起狀態 (-1 代表 Toggle)

    if (A_IsSuspended) {
        ; 當進入掛起狀態時：
        A_TrayMenu.Check(suspendLabel) ; 在選單項目前面打勾
        __ShowTip("熱鍵已暫停 (Suspend ON)", 1000)

        TraySetIcon(trayIcon_suspend, , true) ; 切換托盤圖示
    } else {
        ; 當恢復正常狀態時：
        A_TrayMenu.Uncheck(suspendLabel)
        __ShowTip("熱鍵已恢復 (Suspend OFF)", 1000)

        TraySetIcon(trayIcon) ; 恢復托盤圖示
    }
}
